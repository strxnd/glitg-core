...
Remediation update: 2026-08-15. This matrix is a public clean-room comparison, not a claim of exact equivalence. Closed-source defaults, permissions, messages, and edge cases remain unverifiable without a licensed runtime. Packet-only anti-health-indicator, anti-seed-cracking, and anti-minimap controls are explicitly absent; the former inert switches were removed.
...
| Live feature toggles | Yes | U4 Dec 17 | IMPLEMENTED + MANUAL TEST REQUIRED | Config tests | Listener, service, task, recipe, grace, ritual, and world-rule state use the same feature source |
| Reload and validation | Yes | Repeated bugfixes | IMPLEMENTED + TESTED | Strict config parsing | Full candidate validation plus service/recipe rollback retains prior live state |
...
| Mythical existing-item exemption | Yes | U6 v1.7 | IMPLEMENTED + MANUAL TEST REQUIRED | SQLite migration test | One-time per-player PDC marking, including nested bundle/shulker contents |
...
| Immortal/death-protected items | Yes (“Deathdrop Immunity”) | U4 Feb 1 | IMPLEMENTED + MANUAL TEST REQUIRED | Matcher automatic | Exact drops become invulnerable, non-aging, unlimited-lifetime world entities |
...
| First-join kit | Yes | U5 Oct 24/Oct 30 | IMPLEMENTED + MANUAL TEST REQUIRED | SQLite eligibility test | Exact serialized slots including armor/offhand; returning players are not misclassified |
| Kit save/load/clear/reset/join/give all | Yes | U5 Oct 24/Oct 30 | IMPLEMENTED + MANUAL TEST REQUIRED | Command grammar test | Includes exact `/kit load @a`, legacy give, and durable reset eligibility |
...
| Anti Health Indicator | Yes | Overview; U3 Apr 6 | MISSING | — | Inert packet switch removed; built-in positive health display is a separate feature |
| Anti Seed Cracking | Yes | Overview; U3 Apr 6 | MISSING | — | Inert switch removed; terrain disclosure prevents an unconditional secrecy claim |
| Anti Minimap / fair minimap | Yes | U4 Mar 1/Dec 17 | MISSING | — | Inert switch removed; no fake client enforcement |
| Locator Bar | Yes | U4 Feb 1 | IMPLEMENTED + MANUAL TEST REQUIRED | Paper compile + smoke | Applies and restores the vanilla world game rule |
| XP clumps | Yes | Current overview | IMPLEMENTED + MANUAL TEST REQUIRED | Paper compile + smoke | Configurable nearby-orb aggregation |
| Shield behavior fixes | Yes | Current overview | IMPLEMENTED + MANUAL TEST REQUIRED | Paper compile + smoke | Sound, disable cooldown, and damage-immunity controls |
| One-player sleep | Yes | Current overview | IMPLEMENTED + MANUAL TEST REQUIRED | Paper compile + smoke | Configurable vanilla sleeping percentage |
| Global PvP and damage modifier | Yes | Current overview | IMPLEMENTED + MANUAL TEST REQUIRED | Damage policy tests + smoke | World PvP rule plus general damage multiplier |
| Built-in health indicator | Yes | Current overview | IMPLEMENTED + MANUAL TEST REQUIRED | Paper compile + smoke | Plugin-owned below-name health objective |
...
| Warden Heart | Yes | U2 Jun 6; U4 Feb 1 | PUBLIC SEMANTICS AMBIGUOUS — CLOSEST IMPLEMENTATION PROVIDED | Manual | Right-click, drop, or both acquisition paths; exact proprietary effect not claimed |
...
The matrix documents the known public surface and intentionally records three packet-only features as missing. The implementation must not be described as an exact match until those features work and a licensed black-box comparison verifies the unpublished behavior. Historical removals are excluded and listed separately.
